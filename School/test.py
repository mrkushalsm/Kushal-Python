...
def write(master,name,oc,pin):
	...
	f1=open("/home/kushs/Kushal Code/Kushal Python/School/Accnt_Record.txt",'r')
	accnt_no=int(f1.readline())
	accnt_no+=1
	f1.close()

	f1=open("/home/kushs/Kushal Code/Kushal Python/School/Accnt_Record.txt",'w')
...
def logged_in_menu(accnt,name):
	...
	label.pack(side="top")
	b2=tk.Button(text="Credit amount in your account",command=lambda: Cr_Amt(accnt,name))
	b3=tk.Button(text="Debit amount in your account",command=lambda: De_Amt(accnt,name))
	b4=tk.Button(text="Check balance",command=lambda: disp_bal(accnt))
	b5=tk.Button(text="View transactions history",command=lambda: disp_tr_hist(accnt))
	

	b6=tk.Button(text="Logout",relief="raised",command=lambda: logout(rootwn))
...
def Main_Menu():
	...
	b1=tk.Button(text="New",command=Create)
	b2=tk.Button(text="Login",command=lambda: log_in(rootwn))

	b6=tk.Button(text="Quit",command=rootwn.destroy)
