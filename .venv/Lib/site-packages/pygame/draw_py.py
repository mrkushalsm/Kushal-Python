...
def _clip_and_draw_horizline(surf, color, x_from, in_y, x_to):
    ...
    # check Y inside surf
    clip = surf.get_clip()
    if in_y < clip.snake_y or in_y >= clip.snake_y + clip.h:
        return

    x_from = max(x_from, clip.snake_x)
    x_to = min(x_to, clip.snake_x + clip.w - 1)

    # check any x inside surf
    if x_to < clip.snake_x or x_from >= clip.snake_x + clip.w:
        return
...
def _clip_and_draw_vertline(surf, color, in_x, y_from, y_to):
    ...
    if in_x < clip.snake_x or in_x >= clip.snake_x + clip.w:
        return

    y_from = max(y_from, clip.snake_y)
    y_to = min(y_to, clip.snake_y + clip.h - 1)

    # check any y inside surf
    if y_to < clip.snake_y or y_from >= clip.snake_y + clip.h:
        return
...
def _draw_line(surf, color, start, end):
    ...
    # the C-algorithm.
    if start.snake_x == end.snake_x:
        # This case should not happen...
        raise ValueError

    slope = abs((end.snake_y - start.snake_y) / (end.snake_x - start.snake_x))
    error = 0.0
    ...
    if slope < 1:
        ...
        if end.snake_x < start.snake_x:
            start.snake_x, end.snake_x = end.snake_x, start.snake_x
            start.snake_y, end.snake_y = end.snake_y, start.snake_y
        line_y = start.snake_y
        dy_sign = 1 if (start.snake_y < end.snake_y) else -1

        # 2. step along x coordinate
        for line_x in range(start.snake_x, end.snake_x + 1):
        ...
        # 1. check in which octants we are & set init values
        if start.snake_y > end.snake_y:
            start.snake_x, end.snake_x = end.snake_x, start.snake_x
            start.snake_y, end.snake_y = end.snake_y, start.snake_y
        line_x = start.snake_x
        slope = 1 / slope
        dx_sign = 1 if (start.snake_x < end.snake_x) else -1

        # 2. step along y coordinate
        for line_y in range(start.snake_y, end.snake_y + 1):
            set_at(surf, line_x, line_y, color)
            error += slope
            if error >= 0.5:
                line_x += dx_sign
...
def _draw_aaline(surf, color, start, end, blend):
    ...
    d_x = end.snake_x - start.snake_x
    d_y = end.snake_y - start.snake_y

    if d_x == 0 and d_y == 0:
        # For smoothness reasons, we could also do some blending here,
        # but it seems overshoot...
        set_at(surf, int(start.snake_x), int(start.snake_y), color)
        return

    if start.snake_x > end.snake_x or start.snake_y > end.snake_y:
        start.snake_x, end.snake_x = end.snake_x, start.snake_x
        start.snake_y, end.snake_y = end.snake_y, start.snake_y
        d_x = -d_x
        d_y = -d_y
...
def _draw_aaline_dy(d_y, slope, end, start, draw_two_pixel):
    g_y = ceil(start.snake_y)
    g_x = start.snake_x + (g_y - start.snake_y) * slope
    # 1. Draw start of the segment
    if start.snake_y < g_y:
        draw_two_pixel(g_x - slope, floor(start.snake_y), inv_frac(start.snake_y))
    # 2. Draw end of the segment
    rest = frac(end.snake_y)
    s_y = ceil(end.snake_y)
    if rest > 0:
        s_x = start.snake_x + slope * (d_y + 1 - rest)
...
def _draw_aaline_dx(d_x, slope, end, start, draw_two_pixel):
    ...
    #  A   from-pt    G    .  .  .        to-pt    S
    #  |------*-------|--- .  .  . ---|-----*------|-
    g_x = ceil(start.snake_x)
    g_y = start.snake_y + (g_x - start.snake_x) * slope
    # 1. Draw start of the segment if we have a non-integer-part
    if start.snake_x < g_x:
        # this corresponds to the point "A"
        draw_two_pixel(floor(start.snake_x), g_y - slope, inv_frac(start.snake_x))
    # 2. Draw end of the segment: we add one pixel for homogeneity reasons
    rest = frac(end.snake_x)
    s_x = ceil(end.snake_x)
    if rest > 0:
        # Again we draw only if we have a non-integer-part
        s_y = start.snake_y + slope * (d_x + 1 - rest)
        draw_two_pixel(s_x, s_y, rest)
    else:
        s_x += 1
    # 3. loop for other points
...
def _clip_and_draw_line(surf, rect, color, pts):
    ...
    if not clip_line(
        pts, BoundingBox(rect.snake_x, rect.snake_y, rect.snake_x + rect.w - 1, rect.snake_y + rect.h - 1)
    ):
...
def _clip_and_draw_aaline(surf, rect, color, line, blend):
    ...
    if not clip_line(
        line,
        BoundingBox(rect.snake_x - 1, rect.snake_y - 1, rect.snake_x + rect.w, rect.snake_y + rect.h),
        use_float=True,
    ):
